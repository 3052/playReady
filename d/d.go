...
func new_la(key *ecdsa.PublicKey, cipher_data []byte, kid string) xml.La {
...
func new_envelope(device *c.LocalDevice, kid string) (*xml.Envelope, error) {
   ...
   r, s, err := ecdsa.Sign(a.Fill('D'), device.SigningKey[0], signed_digest[:])
   if err != nil {
      return nil, err
   }
